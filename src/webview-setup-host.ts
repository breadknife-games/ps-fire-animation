...
export const webviewInitHost = ({
    ...
    return new Promise((resolve, reject) => {
        ...
        if (multi === true || Array.isArray(multi)) {
            pages = config.manifest.entrypoints
                .filter(point => (point as any).type === 'panel')
                .map(point => point.id.split('.')!.pop()!)
            console.log('webviewInitHost multi pages', pages)
        }
        let apis: WebviewAPI[] = []
        pages.map((page, i) => {
            let webview = document.createElement(
                'webview'
            ) as UXPHTMLWebViewElement
            ...
            const parent =
                i === 0
                    ...
                    : Array.from(
                          ...
                          item =>
                              item.getAttribute('panelid') === `${id}.${page}`
                      )
            console.log({ parent })
            webview = parent!.appendChild(webview) as UXPHTMLWebViewElement

            webview.addEventListener('message', e => {
                console.log('webview message', page, e)
            })
